...
class RuhiUserBot:
    def __init__(self):
        ...
        self._initializing_groups = set()  # Groups currently being initialized
        self._ignore_cooldowns = {}  # group_id -> last_ignore_reaction_time
        self._reply_locks = {}  # group_id -> asyncio.Lock (prevent duplicate replies)
        self._running = False
    ...
    async def start(self):
        ...
        # Initialize groups in background
    ...
    async def _initialize_all_groups(self):
        """Scrape last 500 messages from all groups on first startup."""
        await asyncio.sleep(3)  # Give Telethon time to fully connect
    ...
    async def _scrape_group_history(self, entity, group_id: int, group_name: str):
        """Scrape the last 500 messages from a group and store them."""
            ...
            async for message in self.client.iter_messages(entity, limit=config.INIT_MESSAGE_COUNT):
                if message.text:
                    ...
                    messages_data.append((
                        ...
                        sender_id,
                        sender_name,
                        message.text[:4000],  # Truncate very long messages
                        timestamp,
                        is_self,
    ...
    async def _handle_new_message(self, event):
        """Handle incoming messages across all chats."""
        try:
            ...
            # Skip if group is still initializing
            if group_id in self._initializing_groups:
                return
            
            # Initialize group if needed (new group joined after startup)
            if not await db.is_group_initialized(group_id):
            ...
            # Process with reply lock to prevent duplicate replies
    ...
    async def _process_and_maybe_reply(
        ...
        """Process a message and decide whether to reply."""
        ...
        # If directly addressed or replied to, always try to respond
        ...
        # Call AI to decide and generate reply
        ...
        if reply:
            ...
            # Periodically update relationship notes via AI
    ...
    def _is_directly_addressed(self, text: str) -> bool:
        """Check if the message is directly addressing the bot."""
        text_lower = text.lower()
        triggers = [
            "ruhi", "रूही", "roohi", "sakib",
            "@ruhi", "ruhi?", "ruhi!", "ruhi,",
    ...
    async def _check_if_ignored(self, group_id: int) -> bool:
        """Check if the bot has been ignored after its last message."""
        try:
            ...
            # Only check if within the threshold window
            ...
            # If there are messages after ours but none reference us, we're being ignored
    ...
    async def _simulate_human_behavior(self, event, reply_text: str, group_id: int):
        """Simulate human-like behavior: reading delay, typing indicator, etc."""
        try:
            chat = await event.get_chat()
            
            # 1. Random initial delay (reading the message)
            ...
            try:
                ...
                pass  # Some chats don't support this
            
            # 3. Small pause after reading
            await asyncio.sleep(random.uniform(0.3, 1.0))
            
            # 4. Show typing indicator for realistic duration
            typing_duration = len(reply_text) / config.TYPING_SPEED_CPS
            typing_duration = max(1.0, min(typing_duration, 15.0))  # Clamp between 1-15 seconds
            ...
            # If simulation fails, add a basic delay anyway
            await asyncio.sleep(random.uniform(2.0, 4.0))
    
    # --- Background Tasks ---
    
    async def _ignore_detection_loop(self):
        """Periodically check if the bot is being ignored in active groups and react."""
        await asyncio.sleep(60)  # Wait before starting
        ...
        while self._running:
            try:
                ...
                for group in groups:
                    ...
                    # Cooldown check - don't spam ignore reactions
                    last_reaction = self._ignore_cooldowns.get(group_id, 0)
                    now_ts = datetime.now(timezone.utc).timestamp()
                    if now_ts - last_reaction < 600:  # 10 minute cooldown
                        continue
    ...
    async def _periodic_prune(self):
        """Periodically prune old messages from the database."""
        await asyncio.sleep(300)  # Wait 5 minutes before first prune
        ...
        while self._running:
            ...
            await asyncio.sleep(3600)  # Prune every hour
    
    async def _update_relationship_notes(self, user_id: int, group_id: int, user_name: str):
        """Background task to update relationship notes via AI."""
        try:
            pool = await db.get_pool()
            async with pool.acquire() as conn:
                # Get recent exchanges between this user and self
